...
/**
 * Componente para mostrar el Top 5 de tokens por precio
 */
export function TopPriceSection() {
  const { t } = useI18n();
  const [tokens, setTokens] = useState<TokenPrice[]>([]);
  const [loading, setLoading] = useState(true);

  // Datos de ejemplo basados en la imagen de mercados
      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-12 py-8 sm:py-12 md:py-16">
        ...
        <div className="bg-[#0a0e27]/50 backdrop-blur-lg border border-white/10 rounded-xl p-8">
          <div className="flex items-center justify-center">
            ...
            <span className='ml-3 text-white/70'>{t('sections.top_price.loading')}</span>
          </div>
        </div>
      </div>
    );
        ...
        <div className="divide-y divide-white/10">
          {tokens.map((token, index) => (
            <div 
              ...
              tabIndex={0}
              role="button"
              aria-label={`Token ${token.name} en posición ${token.rank} por precio`}
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
                ...
                <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between sm:justify-end space-x-4 sm:space-x-0 sm:space-y-1">
                  <div className="text-sm sm:text-lg font-semibold text-white text-right">
                    <span className="block sm:hidden text-xs text-white/60 mb-1">{t('sections.top_price.price_label')}</span>
